...
Kaolin is a popular 3D vision library based on PyTorch. Flux3D.jl is overall faster than Kaolin in terms of applying transforms on PointCloud/TriMesh and comparable with Kaolin in terms of applying metrics. However, there must be work done to improve the back pass in laplacian loss which uses Sparse Arrays. It is evident from the plots of benchmarking that we are easily able to match the performance of Flux3D with Kaolin, and some key-differences between two are also discussed later in this post.
