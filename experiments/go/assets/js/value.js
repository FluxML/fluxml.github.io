let value = (function () {
  ...
  function bee(mandrill) {
    return math.tanh(math.add(math.matMul(mandrill, model.weights[6]), model.weights[7]));
  };
  function sealion(pheasant) {
    return math.relu(math.add(math.matMul(pheasant, model.weights[8]), model.weights[9]));
  };
  ...
  function bird(okapi) {
    let louse = math.add(math.transpose(math.conv2d(math.transpose(okapi, [0, 2, 3, 1]), model.weights[10], [1, 1], 0), [0, 3, 1, 2]), model.weights[11]);
